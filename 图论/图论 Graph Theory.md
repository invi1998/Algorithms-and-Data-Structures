...
![](../img/impicture_20220302_120126.png)

我们上面介绍的dijkstra算法，他是无法处理负权边问题的。这是因为Dijkstra算法是选择每一次我们还没有求到最短路径节点中那个到达路径最短的节点，我们就说这个最短路径就已经是源点到这个节点的最短路径，这是因为其他的节点都需要绕道才能到达这个节点，比如0到2这条路径，如果是其他的路径，比如从0-1-2，或者0-3-2他们这些绕道到达2这个节点，而此时0直接到2这条路径路径是我们考虑的节点中最短的那条，也就是说0-1， 0-3他们的距离都大于0-2，那么这时候，在绕道从1或者3回到2的话，长度只可能更长，所以基于此我们就设立了相对比较高效的dijkstra路径算法。

但是如果我们的图中存在负权边，事情就不一样了。比如这个图，现在有一条从1到2，长度为-4的负权边，那么此时，由于负权边的存在，就有可能使得虽然当前我们从0到1所消耗的费用比0-到2消耗的费用大，但是在绕道的过程中，存在一个负权边，使得我们绕道的这个费用抵消了，换句话说，从绕道从1再到2消耗为-4，那么绕这个道就变得非常的值，使得我们0-1-2这条路径权值为1，小于直接从0到2。

![](../img/impicture_20220302_120930.png)

从这个图中的红色箭头所表示的这条最短路径，看似是在处理负权边，其实本质上还是一次松弛操作，也就是我们发现经过1以后，再转回到2的时候，所获得的这个路径权值是小于不经过1直达2的这条路径权值的，只不过在这条路径上存在了一条负权边而已，但是这依然是一次松弛操作。这也就是说，我们在处理负权边问题的时候，虽然dijkstra算法失效了，但是我们依然要依赖于松弛操作。

在详细讲如何依赖松弛操作处理负权边之前，我们先看负权边会给我们带来什么问题。

![](../img/impicture_20220302_135158.png)

从图中可以看到，我们图中又多了一条从2到0的负权边，那么此时再这个图中就形成了 **负权环**这样的一个路径。从0到1，再从1到2，再从2到0，在这个环中，权值就是5-4-3 = -2，当一个图中出现了负权环会出现什么样的情况？答案就是我们只要从一点到另外一点，只要能够经过这个负权环的话，那么我们想要找到所谓的最短路径的话，我们就会陷入不停的在这个环中转，因为每转一次环，权值就会更少，转的越多，权值越少，永无止境。这样一拉，就导致我们在图中就不存在最短路径了，或者说最短路径的结果是负无穷。

![](../img/impicture_20220302_135416.png)

也正是因为如此，在我们处理负权边问题的时候，要明白，一个图如果拥有负权环，他就不再拥有最短路径了。除此之外，不要局限的认为负权环至少的需要3个节点，其实在这个图中，我们不加2-0这条负权边的话，1-2这两个节点也形成了一个负权环。对于这个图其实也已经不存在最短路径了，因为只要能够到达1,或者2这个顶点就能再这个负权环中不停的转，永无止境。



## Bellman-Ford（贝尔曼-福特算法） 单源最短路径算法

![](../img/impicture_20220302_140602.png)

对于Bellman-Ford单源最短路径算法来说，它解决的问题就是，图中可以有负权边，但是不能有负权环这样的图的单源最短路径问题。

不过Bellman-Ford比我们想象的更加出色，对于不能有负权环这个条件它不是算法运行之前必须遵守的前提。这和我们的二分查找之前必须要求我们的查找数组是有序的这点是不同的。如果我们的这个图中有负权环的话，Bellman-Ford经过运行之后，虽然找不到最短路径，但是能够帮助我们判断图中是否有负权环。相当于我们不用再运行这个算法之前来检测这个图中是否有负权环了。直接在这个算法中转一遍，如果有负权环，Bellman-Ford会告诉我们有负权环，如果没有负权环，这个算法就能求出源点s到其他节点的最短路径。

这样一个算法的代价就是他的时间复杂度相对于Dijkstra算法要高得多。他的时间复杂度是 O(EV)。

尽管如此，如果我们图中不可避免的会产生负权边，Bellman-Ford算法还是非常有意义的一个算法。

接下来，学习一下Bellman-Ford算法的具体思想。

![](../img/impicture_20220302_140845.png)

Bellman-Ford是基于这样一个事实，如果我们在一个图中不存在负权环，那么从某一个源点开始，到另外的一个点，它的最短路径最多只需要经过所有的这v个顶点，也就是说最多有v-1条边。这是因为如果从一个点到另外一个点的最短路径，如果它要经过比V个顶点还要多的顶点（比如说经过了V+1个顶点），那么这个图中总共就只有V个顶点，那是为了求最短路径却需要经过比V还多的顶点，就说明在这个过程中，肯定有节点被访问了不止一次，那么也就可以说明，这个时候，这个最短路径中存在一个环，而我们这个路径是最短路径，但是这个最短路径却存在一个环，一定是因为这个环是一个负权环。

那么这个时候，也就和我们一开始假设的这个图中没有负权环是矛盾的。所以我们就证明了这样一个结论，从医德节点到另外一个节点的最短路径，最多需要经过V-1个点。

在这样一个结论下，我们来看如何通过松弛操作来获得一个最短单源路径。

![](../img/impicture_20220302_142309.png)

可以看到，对于现在的这个图，它拥有两个负权边。一样的我们要想得到从源点0开始到其他所有节点的最短路径，首先我们需要找到和0相邻的所有邻边，找到他的邻边之后，和Dijkstra算法一样，我们就可以说我们暂时找到了从0到和0相邻的这几个相邻节点的路径，并且暂时是一个最短路径。这里我们换一个角度看待我们遍历0的所有邻边这个操作。事实上我们是对0的这个相应的节点进行了一次松弛操作，进行这次松弛操作以后，得到的结果其实是，从源点0开始，经过一条边，到其他节点的最短路径。

然后因为我们现在图中有负权边了，所以这个时候如果我们单独拎出最短的这条从0到2这条边，说到节点2的最短路径就是2，这个说法就不再成立了。虽然我们的图中有负权边，但是我们找到最短路径的关键依然是松弛操作，只不过我们不能非常武断的把最短的那条路径拿出来就说一定它就是最短的了。我们可以尝试对所有的点再次进行松弛操作。也就是说进行第二次松弛操作，那么此时我们可以再换个角度理解一下松弛操作的核心，松弛操作其实就是我们尝试一下从一个点出发能不能经由另外一个点再回到原来的点，来获得一个更短的路径。换句话说，松弛操作从某种程度上就是尝试一下，我们现在找到了一条只有一条边的路径，在这个基础上，我们来看一下有没有只有两条边的路径使得总路径的权值更小

![](../img/impicture_20220302_150849.png)

在我们这个例子中，我们就是找到了这样的一个路径，就是0-1-2这样一个路径，它拥有两条边，可是到2这个节点路径更短。这也就告诉我们了， 如果我们对所有的点再次进行第二次松弛操作的话，我们就可以找到从源点开始，经过两条边相应的最短路径。

通过上面的描述，我们就能明白，我们对一个点进行一次松弛操作，本质上就是找到了经过这个点的另外一条路径。它多了一条边，可是他的权值更小，我们在尝试找是否存在这样的路径。而我们有说了，如果一个图中没有负权环的话，从一个点到另外一个点，最多只需要V-1条边就能找到最短路径。所以如果我们可以堆所有的点都进行V-1次的松弛操作的话，最终就可以找到从这个点到其他所有点的最短路径

![](../img/impicture_20220302_151736.png)

这也就是Bellman-Ford单源最短路径算法的主题框架，就是对所有的点进行V-1次松弛操作。如果进行了V-1次松弛操作以后，我们在尝试多进行一轮松弛操作，也就是对所有的点进行第V轮的松弛操作，如果发现还可以继续松弛的话，那么就可以说明这个图中有负权环。

也正是因为如此，对所有的点都进行一次松弛操作，对每一个边都要遍历一遍，那么这个复杂度是O（E）这个级别的。我们要进行V轮这样的操作，所以整个Bellman-Ford单源最短路径算法的复杂度就是O(EV)；

```c++

template <class Graph, class Weight> class BellmanFord {
...
测试贝尔曼-福特（Bellman-Ford）算法在有没有负权环的情况下，求解源点s的最短单源路径

![](../img/impicture_20220302_164008.png)

测试贝尔曼-福特（Bellman-Ford）算法在有负权环的情况下，求解源点s的最短单源路径

![](../img/impicture_20220302_164202.png)
...
// filename = "testG2.txt";
  ...
  cout << "测试 贝尔曼-福特 算法 Bellman-Ford 求最短单源路径:" << endl << endl;
...
另外有一点非常重要，在我们处理有负权边的时候，通常处理的都是有向图，这是为什么？因为这个图它一旦是一个无向图的话，对于无向图来说，如果从节点a到节点b是一个负权边的话，在无向图中，就相当于从节点b到节点a也有了一个负权边，那么这两个负权边合在一起就相当于有了一个负权环。也正是因为如此，通常在无向图中，我们运行这个bellman-Ford算法，一旦我们的图中有一条负权边，最终的结果都是存在负权环

而如果我们的图中没有负权边，我们就可以使用我们之前的 迪杰斯拉特(Dijkstra)算法，他的时间复杂度会更低一些。

## 更多和最短路径相关的问题

![](../img/impicture_20220302_173225.png)

**DAG中的最短距离**

DAG（有向无环图）中的最短距离可以在[线性](https://so.csdn.net/so/search?q=线性&spm=1001.2101.3001.7020)时间内计算出来。

我们使用拓扑排序来查找单个源到所有其他节点的距离。

Floyd算法又称为插点法，是一种利用 动态规划 的思想寻找给定的 加权图 中多源点之间 最短路径 的算法，与 Dijkstra算法 类似。在计算机科学中，Floyd-Warshall算法是一种在具有正或负边缘权重（但没有负周期（负权环））的加权图中找到最短路径的算法。算法的单个执行将找到所有顶点对之间的最短路径的长度（加权）。 虽然它不返回路径本身的细节，但是可以通过对算法的简单修改来重建路径。 该算法的版本也可用于查找关系R的传递闭包，或（与Schulze投票系统相关）在加权图中所有顶点对之间的最宽路径。

![](../img/impicture_20220302_173449.png)

最长路径算法

![](../img/impicture_20220302_173948.png)